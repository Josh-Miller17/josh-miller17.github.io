...
ggplot(dat.kitsberg.cleaned,
  ...
  theme_bw() +                                
  labs(x = "Sample Type",                       # created labels
       y = "Viruses Within Nucleus",
       fill = "Cell Type",
       title = "Viruses in the Nucleus by Sample Type and Cell Type"
  )
```
...
```{r}
#| label: fig-DWT
#| fig-cap: "Ship Prices vs. Deadweight Tonnage"
#| fig-width: 5
#| fig-height: 3
#| scale: "80%"
#| fig-align: "center"
...
Solution: There is little to no relationship between deadweight tonnage and ship price. The scatterplot shows a  spread of prices across all tonnage levels. The correlation coefficients (Pearson = −0.02, Spearman = 0.09, Kendall = 0.07) are all near zero (however the Pearson coefficient is negative while the other two are positive), indicating a weak relationship There are a few high-price outliers at lower tonnage levels as well, but it doesn't impact the trend line drastically.